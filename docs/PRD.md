...
- **Trust deficit → primary sources.** Committed v1.5 candidate: **async source-grounding** — a post-turn job (same pattern as the safety check) attaches 1–3 primary sources to the turn's load-bearing factual claim via web search, rendered as a quiet sources strip after the reply streams; or on-demand via a per-message "sources?" affordance. Never blocks the reply. Needs cost/latency design before commitment.
